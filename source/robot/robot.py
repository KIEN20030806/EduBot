...
class Robot:
    def __init__(self):
        ...
        self.fw_speed = 70  # move forward speed (80)
        self.fw_speed_start = self.fw_speed/4  # move forward speed
        self.fw_step_time = 0.6
        # speed adjustment rate for 2 wheels when moving straight forward
        self.fw_adjust_rate = 10
        self.min_speed = 30
        self.start_position = 0

        self.turn_speed_start = 60  # 80 # turn speed for turning right or left when start
        self.turn_speed_end = 40  # 60 # turn speed for turning right or left when near ending
    ...
    def _calibrate_speed(self, speed):
        ...
        if abs(z) > angle_error_min:
            self.set_wheel_speed(round(
                m1_speed - z * self.fw_adjust_rate), round(m2_speed + z * self.fw_adjust_rate))
    ...
    def _turn_angle(self, angle):
        ...
        while True:
            ...
            if distance <= end_angle - 10:
                self.set_wheel_speed(-speed_factor * (self.turn_speed_end // 2), speed_factor * (self.turn_speed_end // 2))
            if distance <= end_angle - 3:
                self.set_wheel_speed(-speed_factor * (self.turn_speed_end // 3), speed_factor * (self.turn_speed_end // 4))

            if distance >= end_angle:
                # print('Turn stop: ', current_angle, distance, now - last_time)
                break

            if now - time_start > 2000:
                print('Turn timeout')
                self.stop()
